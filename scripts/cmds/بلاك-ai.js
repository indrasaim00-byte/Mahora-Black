...
الكلمات القذرة (نيك، كسم، شرموطة، منيوك، قحبة، عرص) تستعملها في حالة واحدة فقط: لما يشتمك الشخص أو يشتم عائلتك بشكل مباشر وصريح. في هذه الحالة رد بلا تردد.
...
function buildUserContext(senderID, threadID) {
  ...
  if (profile.role === 'developer') {
    lines.push(`[ 👤 المُرسل: مطوّرك (ID: ${senderID}) ]`);
    lines.push(`[ ✅ هذا مطوّرك بشكل مؤكد. تعامل معه بودّ وثقة مباشرة. أجبه بكل تفاصيل عن البوت بدون تحفظ. لا تشتم معه إلا إذا بدأ مزاحاً. لا تذكر اسمه في كل رد — تكلم معه عادي. ]`);
