...
aliases: ["رانك", "ترتيب", "rank"],
        ...
        onChat: async function ({ usersData, event }) {
                ...
                if (!global._rankExpBuffer) global._rankExpBuffer = new Map();
                const buf = global._rankExpBuffer;

                const entry = buf.get(senderID) || { pending: 0, timer: null };
                entry.pending += 1;
                entry.usersData = usersData;
                buf.set(senderID, entry);

                if (entry.timer) clearTimeout(entry.timer);
                entry.timer = setTimeout(async () => {
                        const e = buf.get(senderID);
                        if (!e || e.pending === 0) return;
                        const toAdd = e.pending;
                        e.pending = 0;
                        e.timer = null;
                        try {
